        ...
        try {
            ...
            if (bulkRequest.numberOfActions() == 0) {
                for (count = 0; count < batchSize && System.currentTimeMillis() < maxBatchEndTIme; count++) {
                    ...
                    if (event == null)
                        break;
                    flatMessage=objectMapper.readValue(event.getBody(), FlatMessage.class);
                    if(docWriteRequestFactory.bolSubmitRequest(flatMessage)) {
                    	 if (bulkRequest.numberOfActions() > 0) {
            ...
            bulkRequest = null;
            log.error("FlatMessage:"+flatMessage);
            return Status.BACKOFF;
